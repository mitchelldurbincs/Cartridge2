...
def _assert_same_state_structure(expected: Any, actual: Any, *, path: str) -> None:
    if isinstance(expected, torch.Tensor):
        ...
        if not torch.equal(actual, expected):
            raise ArtifactValidationError(
                f"{path} values do not match the live learner"
            )
        return
    ...
    def __init__(
        ...
        try:
            ...
            self._scheduler_state = (
                _clone_state_to_cpu(scheduler.state_dict())
    ...
    *,
    ...
    for batch_size in (1, 3):
        observation = torch.linspace(
            -1.0,
            1.0,
            steps=batch_size * obs_size,
            dtype=torch.float32,
            device=device,
        ).reshape(batch_size, obs_size)
        try:
            with torch.no_grad():
                expected_outputs = network(observation)
        except Exception as exc:
            raise ArtifactValidationError(
                f"Live network failed deterministic validation at batch {batch_size}: {exc}"
            ) from exc
        if (
            not isinstance(expected_outputs, (tuple, list))
            or len(expected_outputs) != 2
        ):
            raise ArtifactValidationError(
                "Live network must return exactly (policy_logits, value)"
            )
        try:
        ...
        if not isinstance(actual_outputs, (tuple, list)) or len(actual_outputs) != 2:
            raise ArtifactValidationError(
                "ONNX Runtime must return exactly policy_logits and value"
            )

        for name, expected, actual in zip(
            ("policy_logits", "value"), expected_outputs, actual_outputs
        ):
            if not isinstance(expected, torch.Tensor):
                raise ArtifactValidationError(
                    f"Live network output '{name}' must be a tensor"
                )
            ...
            if actual_array.dtype != np.dtype(np.float32):
                raise ArtifactValidationError(
                    f"ONNX Runtime output '{name}' must use float32, "
...
def export_onnx_artifact(
    ...
    if obs_size != artifact_contract.obs_size:
        raise ValueError(
            f"ONNX export obs_size {obs_size} does not match artifact contract "
            f"{artifact_contract.obs_size}"
        )
    ...
    dummy_input = torch.zeros(2, obs_size, device=device)
    ...
    def _export(temp_path: str) -> None:
        ...
        metadata.update(
            {
                "cartridge.schema_version": str(
                    artifact_contract.model_artifact_schema_version
                ),
                ...
                "cartridge.env_contract_version": str(
                    artifact_contract.env_contract_version
                ),
            }
        )
...
def _validate_learner_state_bytes(
    ...
    *,
    ...
    if isinstance(step, bool) or not isinstance(step, int) or step < 0:
        raise ArtifactValidationError(
            "Learner-state expected step must be a nonnegative integer"
        )
    validate_sha256_digest(config_sha256, field="config_sha256")
    try:
    ...
    for field, expected in expected_scalars.items():
        ...
        if actual != expected or (
            field in {"schema_version", "step"} and isinstance(actual, bool)
        ):
            raise ArtifactValidationError(
    ...
    if learner_state_contract is not None:
        if not isinstance(learner_state_contract, LearnerStateContract):
            raise ArtifactValidationError(
                "learner_state_contract must be LearnerStateContract"
            )
...
def validate_learner_state_artifact(
    ...
    *,
    ...
    try:
        ...
        raise ArtifactValidationError(
            f"Learner-state artifact cannot be read: {exc}"
        ) from exc
...
def restore_learner_state(
    ...
    if sha256_bytes(data) != manifest.learner_state.sha256:
        raise ArtifactValidationError(
            "Learner-state blob SHA-256 does not match manifest"
        )
