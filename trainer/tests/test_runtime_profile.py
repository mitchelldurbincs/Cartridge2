...
def test_runtime_profile_matches_language_neutral_namespace():
    ...
    assert profile.data_dir("/data") == Path(
        "/data/profiles/alphazero_board_v1/connect4/v3"
    )
...
@pytest.mark.parametrize(
    "env_id", ["", "Connect4", "../connect4", "connect/4", "connect.4"]
)
...
def test_catalog_profile_resolution_is_compatible_and_versioned():
    ...
    assert profile == RuntimeProfile("alphazero_board_v1", "connect4", 1)
