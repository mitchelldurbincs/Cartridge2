...
impl GameSession {
    ...
    pub(super) fn apply_decision(&mut self, action: u32, analysis: DecisionAnalysis) -> Result<()> {
        ...
        if let Some(record) = self
            .history
            .iter_mut()
            .find(|r| r.state.revision == previous)
        {
    ...
    pub(super) fn random_analysis(&self, action: u32, legal: &[u32]) -> DecisionAnalysis {
        ...
        analysis.actions = legal
            .iter()
            .map(|&index| ActionAssessment {
    ...
    #[cfg(feature = "onnx")]
    pub(super) fn search_analysis(
        &self,
        result: &mcts::SearchResult,
        diagnostics: &mcts::RootDiagnostics,
        checkpoint: Option<CheckpointIdentity>,
    ) -> DecisionAnalysis {
        let mut analysis = self.decision(result.action, DecisionSource::AlphaZeroMcts);
        let estimate = |value| ValueEstimate {
            value,
            perspective_agent: analysis.actor,
            quantity: ValueQuantity::ExpectedOutcome,
            bounds: Some([-1.0, 1.0]),
        };
        ...
        analysis.actions = diagnostics
            .actions
            .iter()
            .map(|a| ActionAssessment {
                action: ActionReference::Discrete { index: a.action },
                network_prior: Some(a.network_prior),
                search_prior: a.search_prior,
                visit_share: Some(a.visit_share),
                selection_probability: Some(a.selection_probability),
                visits: Some(a.visits),
                q_value: a.q_value,
                expanded: Some(a.expanded),
            })
            .collect();
    ...
    pub fn history(&self, from_revision: Option<u64>, limit: usize) -> HistoryResponse {
        ...
        let first = self
            .history
            .front()
            .map_or(self.revision, |r| r.state.revision);
        let from = from_revision.unwrap_or_else(|| self.revision.saturating_sub(limit as u64 - 1));
        let records: Vec<_> = self
            .history
            .iter()
            .filter(|r| r.state.revision >= from)
            .take(limit)
            .cloned()
            .collect();
        let next_revision = records
            .last()
            .and_then(|r| (r.state.revision < self.revision).then_some(r.state.revision + 1));
        HistoryResponse {
            schema_version: 1,
            session_id: self.session_id.clone(),
...
mod tests {
    ...
    fn records_human_and_bot_decisions_on_their_pre_move_positions() {
        ...
        assert!(matches!(
            history.records[0].decision.as_ref().unwrap().source,
